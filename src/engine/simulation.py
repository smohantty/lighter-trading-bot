...
class SimulationEngine(BaseEngine):
    ...
    def _inject_unlimited_balances(self):
        ...
        logger.info(f"[SIMULATION] Injected unlimited balances: {base_asset}={amount}, {quote_asset}={amount}")
    ...
    async def run(self):
        ...
        logger.info(f"[SIMULATION] Starting continuous mode (tick_interval={tick_interval}s)")
    ...
    def _check_and_simulate_fills(self, current_price: Decimal):
        ...
        for cloid, order in self.pending_orders.items():
            ...
            if should_fill:
                ...
                logger.info(f"[SIMULATION] Order filled: {order.side} {order.sz} @ {fill_price} (cloid={cloid.as_int()})")
    ...
    def _update_balances_on_fill(self, order: LimitOrderRequest, fill: OrderFill):
        ...
        if fill.side == OrderSide.BUY:
            ...
            self.ctx.update_spot_balance(base_asset, current_base + fill.size, current_base + fill.size)
            self.ctx.update_spot_balance(quote_asset, current_quote - quote_amount - fill.fee, current_quote - quote_amount - fill.fee)
        else:
